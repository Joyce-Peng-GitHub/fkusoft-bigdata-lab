...
(async () => {
  ...
  try {
    ...
    await page.getByRole('heading', { name: '城市充电能源监测中心' }).waitFor();
    await page.locator('.kpis').waitFor();
    assert.equal(await page.locator('.chart').count(), 13);
    assert.equal(await page.locator('.chart').evaluateAll(charts => charts.filter(chart => chart.querySelector('canvas')).length), 13);
    ...
    assert.match(await page.getByRole('alert').innerText(), /保留上次/);
    assert.equal(await page.locator('.chart').count(), 13);
    assert.equal(await page.locator('.chart').evaluateAll(charts => charts.filter(chart => chart.querySelector('canvas')).length), 13);
    await page.reload();
    await page.getByRole('alert').waitFor();
